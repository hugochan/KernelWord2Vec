...
cdef int ONE = 1
cdef REAL_t NEG_ONEF = <REAL_t>-1.0
...
cdef unsigned long long fast_sentence_sg_neg(const REAL_t gamma,
    const int negative, np.uint32_t *cum_table, unsigned long long cum_table_len,
    REAL_t *syn0, REAL_t *syn1neg, const int size, const np.uint32_t word_index,
    const np.uint32_t word2_index, const REAL_t alpha, REAL_t *work,
    unsigned long long next_random, REAL_t *word_locks, REAL_t *diff) nogil:

    cdef long long a
    cdef long long row1 = word2_index * size, row2
    cdef unsigned long long modulo = 281474976710655ULL
    cdef REAL_t f, g, label, kernel, ng
    ...
    for d in range(negative+1):
        ...
        scopy(&size, &syn0[row1], &ONE, diff, &ONE) # copy syn0[row1:row1+size-1] to diff
        our_saxpy(&size, &NEG_ONEF, &syn1neg[row2], &ONE, diff, &ONE) # diff = diff - syn1neg[row2:row2+size-1]
        f = our_dot(&size, diff, &ONE, diff, &ONE)
        kernel = exp(-gamma * f) # kernel

        if kernel <= -MAX_EXP or kernel >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((kernel + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = 2 * gamma * kernel * (label - f) * alpha
        ng = -g
        our_saxpy(&size, &ng, diff, &ONE, work, &ONE)
        our_saxpy(&size, &g, diff, &ONE, &syn1neg[row2], &ONE)
...
def train_batch_sg(model, sentences, alpha, _work):
    ...
    cdef REAL_t *diff = <REAL_t*> PyMem_Malloc(size * cython.sizeof(REAL_t))
    if not diff:
        raise MemoryError()
        ...
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                ...
                for j in range(j, k):
                    ...
                    if negative:
                        next_random = fast_sentence_sg_neg(gamma, negative, cum_table, cum_table_len, syn0, syn1neg, size, indexes[i], indexes[j], _alpha, work, next_random, word_locks, diff)
