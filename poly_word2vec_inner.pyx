...
cdef unsigned long long fast_sentence_sg_neg(const REAL_t gamma,
    ...
    cdef REAL_t f, g, label, kernel, dot_val
    ...
    for d in range(negative+1):
        ...
        # for polynominal (d=2) word2vec, simply replace the original dot product with kernel function (x * y + c)^2
        dot_val = our_dot(&size, &syn0[row1], &ONE, &syn1neg[row2], &ONE) + gamma # dot product (with bias)
        kernel = dot_val * dot_val
        if kernel <= -MAX_EXP or kernel >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((kernel + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]

        g = 2 * dot_val * (label - f) * alpha
