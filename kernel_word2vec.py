...
from numpy import exp, log, dot, zeros, outer, random, dtype, float32 as REAL,\
    ...
    array, ndarray, empty, sum as np_sum, prod, ones, ascontiguousarray, vstack
...
def fast_sigmoid_array(x):
    return array([fast_sigmoid(d) for d in x])

sigmoid_table = init_sigmoid_table()

try:
    # from kernel_word2vec_inner import train_batch_sg#, train_batch_cbow
    # from kernel_word2vec_inner import score_sentence_sg#, score_sentence_cbow
    # from kernel_word2vec_inner import FAST_VERSION, MAX_WORDS_IN_BATCH
    from poly_word2vec_inner import train_batch_sg#, train_batch_cbow
    from poly_word2vec_inner import score_sentence_sg#, score_sentence_cbow
    from poly_word2vec_inner import FAST_VERSION, MAX_WORDS_IN_BATCH
except ImportError:
...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
    ...
    if model.negative:
        ...
        # for kernel word2vec, simply replace the original dot product with kernel function
        kernel = gaussian_kernel(l1, l2b, model.gamma)
        fb = fast_sigmoid_array(kernel)
        # fb = 1. / (1. + exp(-kernel))  # propagate hidden -> output
        gb = 2 * model.gamma * kernel * (model.neg_labels - fb) * alpha  # vector of error gradients multiplied by the learning rate
        if learn_hidden:
            model.syn1neg[word_indices] += gb.reshape(model.negative + 1, 1) * (l1 - l2b)  # learn hidden -> output
        neu1e += dot((l2b - l1).T, gb)  # save error
...
def gaussian_kernel(x, y, gamma):
    return exp(-gamma * ((x - y)**2).sum(1))

def gaussian_kernel_pair(x, y, gamma):
    dist = x - y
    return exp(-gamma * dot(dist, dist.T))

def poly_kernel_pair(x, y, c=0, d=2):
    return (dot(x, y) + c)**d
...
class Word2Vec(utils.SaveLoad):
    ...
    """
    def __init__(
            self, sentences=None, size=100, gamma=20, alpha=0.025, window=5, min_count=5,
    ...
    def save_word2vec_format(self, fname, fvocab=None, binary=False):
        ...
        logger.info("storing param settings into %s.params" % fname)
        with utils.smart_open("%s.params" % fname, 'wb') as fout2:
            json.dump({'size': self.vector_size, 'gamma': self.gamma, 'window': self.window, 'min_count': self.min_count, \
                    'sg': self.sg, 'hs': self.hs, 'negative': self.negative, 'iter': self.iter}, fout2)
    ...
    def load_word2vec_format(cls, fname, fvocab=None, binary=False, encoding='utf8', unicode_errors='strict',
        ...
        params = None
        with utils.smart_open("%s.params" % fname) as fin2:
            params = json.load(fin2)
            result.gamma = params['gamma']

        if params:
            logger.info("loaded param settings from %s.params" % fname)
        else:
            logger.info("warning: failed to load param settings from %s.params" % fname)
    ...
    def similarity(self, w1, w2):
        """
        Compute cosine similarity (in hyperspace) between two words.
        ...
        # return gaussian_kernel_pair(self[w1], self[w2], self.gamma)

        k12 = poly_kernel_pair(self[w1], self[w2], self.gamma)
        k11 = poly_kernel_pair(self[w1], self[w1], self.gamma)
        k22 = poly_kernel_pair(self[w2], self[w2], self.gamma)
